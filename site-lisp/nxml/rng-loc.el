...
(defun rng-set-document-type ()
  ...
  (condition-case err
      (let* ((type-ids
	      (rng-uniquify-equal (sort (rng-possible-type-ids) 'string<)))
...
lost when the buffer is killed."
  ...
  (condition-case err
      (progn
	(rng-set-schema-1 filename)
	(rng-save-schema-location-1 t))
    (nxml-file-parse-error
     (nxml-display-file-parse-error err))))

(defun rng-set-schema-1 (filename)
  (setq filename (expand-file-name filename))
  (setq rng-current-schema (rng-load-schema filename))
...
(defun rng-auto-set-schema ()
  ...
  (interactive)
  (condition-case err
      (let ((schema-file (rng-locate-schema-file)))
	(cond (schema-file
	       (rng-set-schema-1 schema-file))
	      (t
	       (setq rng-current-schema rng-any-element)
	       (setq rng-current-schema-file-name nil)
	       (run-hooks 'rng-schema-change-hook)))
	(rng-what-schema))
    (nxml-file-parse-error
     (if (interactive-p)
...
(defun rng-match-transform-uri-rule (props)
  (let ((path-suffix (cdr (assq 'pathSuffix props)))
	(replace-path-suffix (cdr (assq 'replacePathSuffix props)))
	(path-append (cdr (assq 'pathAppend props)))
	(file-name (buffer-file-name)))
    (and file-name
	 (or (not path-suffix)
	     (rng-file-name-has-path-suffix-p file-name path-suffix))
	 (cond (replace-path-suffix
		(and path-suffix
		     (not (string-match "/" replace-path-suffix))
		     (let ((new-file-name
			    (concat (substring file-name
						  ...
						  (length path-suffix)))
				    replace-path-suffix)))
		       (and (file-exists-p new-file-name)
			    (rng-file-name-uri new-file-name)))))
	       (path-append
		(let* ((components
			(rng-uri-split (rng-file-name-uri file-name)))
		       (uri
			(and components
			     (rng-uri-join (nth 0 components)
					   (nth 1 components)
					   (concat (nth 2 components)
						   path-append)
					   (nth 3 components)
					   (nth 4 components))))
		       (new-file-name
...
(defun rng-match-uri-rule (props)
  (let ((resource (cdr (assq 'resource props)))
	(path-suffix (cdr (assq 'pathSuffix props)))
	(file-name (buffer-file-name)))
    (and file-name
	 (cond (resource
		(condition-case err
		  ...
		  (error nil)))
	       (path-suffix
		(rng-file-name-has-path-suffix-p file-name
						 path-suffix)))
	 (rng-match-default-rule props))))

(defun rng-file-name-has-path-suffix-p (file-name path-suffix)
  (setq file-name
	(file-name-nondirectory file-name))
  (eq (compare-strings path-suffix
		       0
		       nil
		       file-name
		       (max 0
...
attributes are removed."
  (unless rng-schema-locating-file-schema
    (setq rng-schema-locating-file-schema
	  (rng-load-schema rng-schema-locating-file-schema-file)))
  (let* ((element (rng-parse-validate-file rng-schema-locating-file-schema
					   file))
